...
export function registerOnlineInputHandlers(
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (isSelectionPhase(state.phase)) {
      handleKeySelection(e, state, deps);
      return;
    }
...
function handleKeyLifeLost(
  ...
  const { getLifeLostDialog, getControllers, isHuman, sendLifeLostChoice } =
    deps;
  ...
  for (const ctrl of getControllers()) {
    ...
    const entry = lifeLostDialog.entries.find(
      (en) =>
        en.playerId === ctrl.playerId && en.choice === LifeLostChoice.PENDING,
    );
    if (!entry) continue;
    const action = ctrl.matchKey(e.key);
