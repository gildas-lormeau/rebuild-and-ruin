...
const ENTER_UPGRADE_PICK: Transition = {
  ...
  mutate: (ctx) => {
    ...
    // Build the dialog HERE (not just in postDisplay) so it exists before
    // the banner's B-snapshot: the overlay paints the pick cards whenever
    // the dialog is non-null (with a banner-coordinated fade mask), so the
    // banner sweeps to reveal them instead of having them pop in at banner
    // end. postDisplay's prepare() is an idempotent re-check.
 ...
 *  to continue the flow. `prepare()` is idempotent — the mutate already
 *  built the dialog (from the offers `prepareNextRound` generated) so the
 *  banner's B-snapshot shows the cards; this call is a re-check that
 *  doubles as the no-offers guard.
 *
 ...
 *  re-snapping the camera. Two callers:
 *  - the runner's own `route` chaining (prep → entry, same tick) — inside
 *    the `Mode.TRANSITION` window the outer `runTransition` opened, so the
 *    chain's first banner consumes the outer prime;
 *  - `finishUpgradePick` resuming into `enter-wall-build` when the
 *    self-driving picker resolves — dispatched from `tickUpgradePickPhase`
 *    in Mode.UPGRADE_PICK with NO prime pending, so the build banner's
 *    prev-scene falls back to the current display pixels (the last-painted
 *    picker frame), exactly as `showBanner` does for any unprimed banner.
 *  Skipping the prime is correct either way; only the fallback source
 *  differs. */
