...
async function handleKeyF1(
  ...
  if (mode === Mode.LOBBY) {
    ...
    // ticking mode). Fine locally — but online every other peer keeps
    // mirror-ticking: their life-lost grace backstop force-ABANDONs this
    // frozen player's pending dialog entry while the frozen peer later
    // resolves CONTINUE, permanently forking the dialogs. Gated on the
    // session being online at all, not on seated remote humans —
    // spectators are invisible to clients by design and mirror-tick too.
    // Same rule as `togglePause` (subsystems/options.ts) — online has no
    // freeze-recovery. Consume the key without action (the gate runs
    // BEFORE setContext so no stale "gameplay" context leaks into a
    // later lobby-mode show()).
