...
- **Sudden death:** if 2+ alive players share the top score when the round limit is reached, the game does NOT end — a full extra round is played (Cannon Place → Battle → Wall Build), announced by a "Sudden Death" banner before cannon placement. Re-checked at every subsequent round end: the match ends when the top score is unique (that player wins) or eliminations leave one player standing. Everyone alive plays overtime rounds, so a non-tied player who overtakes during sudden death wins. Overtime rounds get no upgrade offers (offers are drawn before the closing round's scores are known). Only the degenerate case where ALL players are dead resolves an exact tie by slot order (Red > Blue > Gold) — there is no one left to play overtime.
