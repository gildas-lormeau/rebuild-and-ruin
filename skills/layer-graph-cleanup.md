...
| Entry-point shift left names stale (May 2026) | After unrelated refactors, `entry.ts` cascaded L1→L2 (gained `online-router` dep) and `main.ts` cascaded L16→L17. L18 was created as `online-client.ts`'s home with the placeholder `name: "layer 18"` and no `tier`. Five renames + one tier add to restore name honesty: L1 "foundational types & local entry" → "foundational types" (entry.ts left); L2 "derived types" → "derived types & local entry" (entry.ts arrived); L12 "phase orchestration & app entry" → "phase orchestration" (main.ts left); L17 "online composition root" → "app entry" (now holds `main.ts` + `online-runtime-game.ts`); L18 placeholder → "online client entry" with `tier: "roots"`. CLAUDE.md narrative updated in the same commit. |
| Phase-flip centralization cascaded names stale again (May 2026) | The `setPhase` centralization refactor dropped `runtime-phase-machine.ts`'s deepest import several layers, which cascaded `main.ts` from L17 to L13 and shrank L15/L16/L17 to one file each. L18 vanished entirely. Four renames to restore name honesty: L13 "wiring" → "wiring & local entry" (main.ts arrived); L15 "session & runtime lifecycle" → "online deps wiring" (just `online-runtime-deps.ts` — server message dispatch + dep-object builders); L16 "app wiring" → "online runtime composition" (just `online-runtime-game.ts` — GameRuntime creation); L17 "app entry" → "online client entry" (just `online-client.ts`, restoring its pre-shuffle name now that main.ts lives at L13). CLAUDE.md updated to "18 groups, L0–L17"; sister skills (import-hygiene, architecture-audit) updated to drop stale L18 references. |
| Shape-duplicate cleanup pushed `protocol.ts` up one layer (May 2026) | Adding `OpponentXxxMessage extends *Payload` (from `phantom-types.ts`) gave `protocol.ts` a downward import to L3, cascading it L3→L4. L3 lost its only "wire format" file (`protocol.ts`); L4 gained it but never actually held a "server stub". Two renames to restore name honesty: L3 "wire format & config types" → "wire payloads & shared definitions" (now holds `cannon-mode-defs`, `phantom-types`, `player-types`, `settings-ui` — payloads + domain-type registries, no envelope); L4 "core game state & server stubs" → "core state & adjacent types" (now holds `protocol`, `online-types`, `system-interfaces`, `types`, `overlay-types`, plus spatial/player helpers — heterogeneous "core state + cross-cutting contracts"). CLAUDE.md narrative updated in the same commit. |
| Value-extraction at L5 pushed `online-client.ts` back to L18 (May 2026) | After extracting value exports out of three `-types`-named files (`toCannonMode` → `cannon-mode-defs`; `CROSSHAIR_SPEED`/`isMovementAction`/`isHuman`/`isAiAnimatable` out of `system-interfaces`; three `interior*` helpers out of `overlay-types`), the cascade re-grew the stack to 19 layers. `online-client.ts` ended up at L18 with the placeholder `name: "layer 18"` and no `tier`, and L15/L16/L17 names had drifted out of alignment with their files. Six renames + one tier add to restore name honesty: L13 "wiring & local entry" → "wiring" (main.ts left); L14 "composition roots" → "composition roots & local entry" (main.ts arrived); L15 "online deps wiring" → "online session lifecycle" (holds `controller-ai-assisted-human`, `online-runtime-promote`, `online-runtime-session` — session-lifecycle files); L16 "online runtime composition" → "online deps wiring" (just `online-runtime-deps.ts`); L17 "online client entry" → "online runtime composition" (just `online-runtime-game.ts`); L18 placeholder → "online client entry" with `tier: "roots"` (just `online-client.ts`). |
| Modifier/upgrade-types drain shifted L7 from "junk drawer" to render-dominated (May 2026) | After moving 11 type symbols out of `modifier-types.ts` + `upgrade-types.ts` into `types.ts` (and deleting the now-empty files), 29 modifier+upgrade impls dropped from L7→L6. L7 shrank from 60→31 files; the residual is 19 per-entity/effect 3D renderers in `src/render/3d/` + 12 cross-domain integration files (`controller-human`, `game-init`, `zone-recompute`, online-host/send/server/stores, `runtime-battle-anim`, `runtime-contracts`, `board-occupancy`, `server/room-manager`, `fire.ts`). Renamed L7 "cross-domain handlers" → "entity renderers & cross-domain handlers" — follows the L6 "upgrades, modifiers & runtime contracts" pattern of leading with the dominant cluster (19/31 = 61% render) while preserving the residual catch-all. CLAUDE.md narrative updated in the same commit. |
