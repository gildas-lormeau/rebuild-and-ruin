...
const MODERN_STATE_EXCLUSIONS: Record<string, string> = {
  ...
  pendingSupplyBonuses:
    "scaffolding — per-player queue of supply-ship bonuses pending consumption. Cross-peer parity holds via mirror-simulation (deterministic hit attribution from same RNG + tick); wire payload lands alongside supplyShips in the follow-up commit so late-joining watchers can restore mid-round bonus state.",
};
