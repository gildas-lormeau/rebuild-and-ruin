...
async function run(): Promise<void> {
  ...
  // `sc.state.round` here = state at the moment runGame stopped. state.round
  // advances inside `resolveAfterLifeLost` (round-end postDisplay), AFTER the
  // life-lost dialog resolves and BEFORE the game-over check. So for a
  // `rounds: N` recording, "final round" is N+1.
  console.log(
