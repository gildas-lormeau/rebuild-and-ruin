...
const TOOLS: ToolDef[] = [
  ...
  },
  {
    name: "build_toward",
    description:
      "WALL_BUILD: hand the whole build phase to the harness with one goal — enclose a tower (default: your home tower). It places each piece that arrives on the best min-cut tile (reacting to pieces, never peeking ahead), redirecting dud pieces onto the ring, until the tower seals, build time runs low, or it stalls. When the last gap is a sub-piece island that needs a smaller piece (or a mobile grunt that may wander off), it keeps cycling the bag toward a fitting draw instead of giving up — but each cycled dud packs ~4 wall tiles into your castle, so the moment the cycling costs any draw-pool shape its last legal placement it stops with outcome 'cycle-risk' (bag-lock brake: skip the seam, place by hand, or declutter next battle). lastResult splits 'placed N (k on-target, m cycled)' whenever duds were cycled — cycled pieces are bag-burn, NOT seal progress; the candidates' estSeconds already prices the expected small-piece wait, so trust a big estimate over retrying. One call instead of dozens of place_piece calls. With NO maxSeconds it self-caps (≈ the seal estimate + buffer) so one big enclosure can't gamble the whole phase — it pauses with progress banked (outcome 'auto-paused'); just call it again to continue. Pass maxSeconds / maxPieces to STOP EARLY and reserve the rest of the phase for a second build. Read lastResult for the outcome (done/time/auto-paused/sec-budget/piece-budget/stuck/blocked/diverging/cycle-risk + gaps left + seconds spent).",
    inputSchema: {
      type: "object",
      properties: {
        towerIdx: {
  ...
  },
  {
    name: "seal_survivor",
    description:
      "WALL_BUILD, when the ☠ SURVIVAL line warns you'll lose a life: ONE call to save it. It seals a compartment around your cheapest survivable tower — the harness places every piece on the min-cut, carving a tight pocket even when your whole outer ring is too big to close in time. No coordinates, no tower pick, no grid-reading. Reach for this the moment you see the SURVIVAL warning instead of passing (which forfeits the life) or hand-building. If no life-clearing tower can be sealed in the time left it does nothing and lastResult says why (the loss is unavoidable) — then spend the time on offense/pre-claim instead. If the only route is a long-odds seal (walls fit but the small-piece bag-wait doesn't), it attempts anyway and disables the normal cycle-risk brake — a lucky small draw saves the life, but each cycled dud packs ~4 wall tiles into your castle first; lastResult flags 'cycled N duds ... declutter next battle' when this happened, since it can set up a bag-lock on a LATER build even though it saved THIS round's life. To keep you in control of that gamble, a survival call is CHUNKED to ~5s and then auto-pauses (so it can't dud-cycle the whole build blind): re-read the fat/pocket in the observation and either call seal_survivor again to keep gambling, declutter to reopen the pocket, or pass to accept the loss. Re-calling is free (the clock is frozen while you think and pieces cost the same either way), so the chunk is purely a re-decision point, not a limit on how hard you can try. Read lastResult for the outcome.",
    inputSchema: {
      type: "object",
      properties: {
        maxSeconds: {
  ...
  },
  {
    name: "declutter",
    description:
      "BATTLE (self-maintenance): fire YOUR OWN battery at YOUR redundant inner ('fat') walls to shoot a build pocket back open — the structural sibling of cull. Cull clears the GRUNTS that box a reseal; declutter clears the WALLS that box your piece bag. Over-building (repeated build_out) packs a castle into single-tile seams until a dealt S/Z/C/+ piece has ZERO legal placements — a BAG-LOCK that forfeits the build and, with no alive tower enclosed, costs a LIFE. Walls can't be removed in WALL_BUILD, but a cannonball CAN shoot one out in BATTLE, so declutter is the proactive escape: it targets only NON-LOAD-BEARING fat (every 8-neighbour is your own wall/interior, so YOUR OWN shots never break an enclosure), skips tower-occluded tiles a ball can't reach, and clears a CONTIGUOUS block so you get one usable 2×2+ dump pocket. Note this makes YOUR fire enclosure-safe, NOT the action: like any battle attack it ties up the whole window, so incidental enemy fire can still breach your OUTER ring while it runs — lastResult reports it, reseal first next build if so. It scores NOTHING (own walls) — a deliberate tempo trade of battle offense for the build room that prevents the lock. Use it the battle BEFORE you'd otherwise pack tight (watch observation.fatClearable / a climbing compactness fat/100). Same live-gated, reload-paced fairness as bombard. Read lastResult for fat cleared + tiles freed + enemy fire.",
    inputSchema: {
      type: "object",
      properties: {
        quanta: {
  ...
  },
  {
    name: "finish_it",
    description:
      "BATTLE (offensive finisher): the 'finish it' attack — glide the cursor all the way AROUND the outer wall of the largest THIN-walled opponent castle and spam MANY small SPACED holes (single-tile gaps, an intact wall always left between distinct gaps — NOT one big breach). Where the wall is single-layer one hole opens the seal; where it is two-thick the shot punches THROUGH the layer behind, and spur gaps are bridged via inner-wall pivots — so the sweep stays fluid and breaches everywhere. At this stage that castle is usually a MERGED enclosure (one seal around several towers): this 'finishes' it ONLY when you open more gaps than the victim can reseal in one build — then ALL its towers de-enclose at once (a big/messy thin castle has too many gaps to patch, which is exactly why it's the target). Every gap is also its own refill (a repair tax, and in modern the volume feeds wall-demolition COMBOS). Auto-targets the largest, thinnest-perimeter enemy (thick 'fat-walled' perimeters resist breaching, so they are AVOIDED, not preferred); no coordinates. Best spent with OVERWHELMING firepower (lots of cannons): the more guns, the more of the ring you breach in the battle window. Does nothing if no opponent is large enough (interior ≥130) — bombard/breach a smaller target instead. Same live-gated, reload-paced fairness as bombard (no clock exploit). Read lastResult for holes punched + which slot + enemy fire.",
    inputSchema: {
      type: "object",
      properties: {
        quanta: {
