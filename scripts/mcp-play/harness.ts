...
export interface Observation {
   ...
   *  one with pick_upgrade({ cardIdx }) where cardIdx is 0, 1, or 2
...
export interface McpGame {
   ...
   *  wall/interior, so declutter's OWN shots can NEVER connect interior to outside
   *  (the enclosure is safe from MY fire by construction). This does NOT make the
   *  ACTION breach-proof: it ties up the whole battle window like any attack, so
   *  incidental enemy fire can still open my OUTER ring while it runs — reported
   *  by `battleSelfReport`, reseal first next build if so. Aim-occluded fat (behind a tower) is
...
export async function createMcpGame(
   ...
   *  reseal; declutter clears the walls that box the piece bag). Only non-load-bearing
   *  fat is shot (see `declutterFatTargets`) so MY fire never de-encloses me — though
   *  the action still eats the battle window's incidental enemy fire, which can breach
   *  my outer ring (see `battleSelfReport`); the clearance is contiguous, and it
...
function expectedFor(phase: Phase): string {
  switch (phase) {
    case Phase.CASTLE_SELECT:
      return "Pick a home tower — call select_castle({ towerIdx }).";
