...
const files = [
  ...globSync("src/**/*.ts", {
    cwd: root,
    // Sprite scene files embed ASCII map variants as string arrays
    // (". . . ." rows, "..#..." etc.) and raw canvas pixel sizes (192,
    // 64…). They're sandbox authoring files, not game code — the
...
const stringResults = applyFileScope(
  [...stringCounts.entries()].filter(([, v]) => v.count >= threshold),
).sort((a, b) => b[1].count - a[1].count) as [
  string,
  { count: number; locations: string[] },
][];
const numberResults = applyFileScope(
  [...numberCounts.entries()].filter(([, v]) => v.count >= threshold),
).sort((a, b) => b[1].count - a[1].count) as [
  number,
  { count: number; locations: string[] },
][];
...
const newStrings = stringResults.filter(
  ([k]) => !baseline.has(baselineKey("string", k)),
);
const newNumbers = numberResults.filter(
  ([k]) => !baseline.has(baselineKey("number", k)),
);
const knownStrings = stringResults.filter(([k]) =>
  baseline.has(baselineKey("string", k)),
);
const knownNumbers = numberResults.filter(([k]) =>
  baseline.has(baselineKey("number", k)),
);
...
function visit(
  ...
  if (ts.isStringLiteral(node) && !isSkippedString(node, sourceFile)) {
    ...
    !NUMERIC_EXCLUDED_FILES.has(path.basename(filePath)) &&
    !isSkippedNumber(node, sourceFile)
  ) {
    record(
      numberCounts as Map<
        string | number,
...
function isSkippedString(
  ...
  if (
    ...
    ts.isNoSubstitutionTemplateLiteral(
      node as ts.Node as ts.NoSubstitutionTemplateLiteral,
    )
  )
    return true;
...
function isDefinitionContext(
  ...
  if (ts.isPropertyAssignment(parent) && parent.initializer === node) {
    ...
    while (cursor.parent) {
      ...
      if (
        ts.isPropertyAssignment(ascending) &&
        ascending.initializer === cursor
      ) {
        cursor = ascending;
        continue;
      }
      if (
        ts.isVariableDeclaration(ascending) &&
        ascending.initializer === cursor
      ) {
  ...
  if (ts.isArrayLiteralExpression(parent)) {
    ...
    if (
      ts.isVariableDeclaration(arrParent) &&
      arrParent.initializer === parent
    ) {
  ...
  if (ts.isArrayLiteralExpression(parent)) {
    ...
    if (ts.isArrayLiteralExpression(outerArr)) {
      ...
      if (
        ts.isVariableDeclaration(outerParent) &&
        outerParent.initializer === outerArr
      ) {
...
function record(
  map: Map<string | number, { count: number; locations: string[] }>,
  key: string | number,
  filePath: string,
  node: ts.Node,
  sourceFile: ts.SourceFile,
...
function applyFileScope(
  ...
  return entries
    .map(([key, { locations }]) => {
      ...
      return [key, { count: filtered.length, locations: filtered }] as [
        string | number,
        { count: number; locations: string[] },
      ];
    })
...
if (updateBaseline) {
  ...
  console.log(
    `Baseline updated: ${total} entries saved to .readonly-literals-baseline.json`,
  );
  process.exit(0);
}
...
if (showAll) {
  ...
  if (knownCount > 0 && newCount === 0) {
    console.log(
      `${knownCount} known duplicate literals (in baseline). Run with --all to see them.`,
    );
