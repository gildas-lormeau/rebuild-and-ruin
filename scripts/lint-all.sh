...
(
  ...
  run phase-graph       deno run -A scripts/gen-phase-graph.ts --check
  ...
  run callback-inv      deno run -A scripts/lint-callback-inversion.ts
  ...
  run monotone-state    deno run -A scripts/lint-monotone-state.ts
  ...
  run helper-reuse      deno run -A scripts/lint-helper-reuse.ts
