...
| `prevCastles`, `prevTerritory`, `prevWalls`, `prevEntities` | Scene snapshot taken at `showBanner(preservePrevScene=true)` call time. |
| `newTerritory`, `newWalls` | Snapshot of the target battle state (entering BATTLE only). |
| `wallsBeforeSweep` | Pre-sweep wall set stashed right before `finalizeBuildPhase` so the "before" image shows unswept walls. |
| `modifierDiff` | `{ id, label, changedTiles, gruntsSpawned }` for modifier reveals. |

`prevCastles`, `prevTerritory`, `prevWalls`, and `prevEntities` are cleared
when the banner ends — they exist only for the duration of the sweep.
...
1. **`showBanner(text, onDone, preservePrevScene?, newBattle?, subtitle?)`**
   ...
   `Mode.BANNER`, and (when `preservePrevScene` is true) snapshots the current
   castles / territory / walls / entities. Fires haptics + sound once. Flags
   `pendingStartEvent = true` — the bus event fires on the **next** tick, not
   here, so that mid-frame mutations (like a modifier reveal replacing the
   battle banner text) land in the event payload.
...
### Modifier banner mid-frame swap

Modifier reveals don't have their own `showBanner` call. Instead
[`runtime-phase-ticks.ts`](../src/runtime/runtime-phase-ticks.ts) does this
inside the BATTLE phase transition:

1. `showBanner("ATTACK!", proceedToBattle)` queues the phase banner.
2. `enterBattleFromCannon(state)` rolls the modifier and returns a
   `ModifierDiff`.
3. If `modifierDiff` is non-null, the code **overwrites** `banner.text`,
   `banner.subtitle`, and `banner.modifierDiff` **in place**, and rewires
   `banner.callback` to chain the original battle banner as a follow-up.

Because `pendingStartEvent` defers the `BANNER_START` emit until the first
tick, the event sees the final post-swap content — tests that listen for
`modifierId` on `bannerStart` don't see a "ghost" battle banner first.
...
2. **`drawBannerPrevScene`** runs next. It:
   ...
   - Caches the banner canvas by reference identity on
     `{ map, castles, territory, walls, modifierTiles }`. The expensive pass
...
- **Forgetting the mid-frame swap**: if you see a `bannerStart` event with
  no `modifierId` in a modern-mode battle transition, you're looking at the
  event before the swap landed. But this can't happen — the swap runs before
  the first tick, and `BANNER_START` fires on the first tick. If a future
  refactor moves the swap after tick 1, this invariant breaks.
...
- **Clearing snapshots early**: `clearSnapshots()` only wipes
  `wallsBeforeSweep` and `prevCastles` (it exists for selection resets, not
  banner teardown). The full snapshot set is cleared by `tickBanner` at
  `progress === 1`.
