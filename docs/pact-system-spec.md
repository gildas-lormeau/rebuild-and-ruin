...
Origin: team-up bonus brainstorm, 2026-07-16. Revised 2026-07-16 (fresh-eyes redesign pass): positional single-press commit replaces the dialog (last-press-wins, per-seating echo) and the seating gate is removed; v1 no-repeat rotation rule; Materiel Convoy added to the extended pool. This spec pins the rules; tuning values are marked TBD.
...
- **No seating gate.** The commit step is a positional single-press (see Commit below) whose echo goes blind on multi-human clients, so it stays secret with 2–3 humans on one keyboard — there is no cursor to watch and no presses to count; finger-glancing is ordinary couch-game physics with one-round stakes. Touch input is only private on a private screen (a tap on a shared display would be watchable), but in practice touch = mobile = one human per screen, and multi-human single-device play is keyboard-driven (`PLAYER_KEY_BINDINGS`); the degenerate "second human taps a shared screen" configuration occurs only in dev and is accepted, not gated.
...
3. **Commit (step 2)** — `PACT_COMMIT`, a new conditional self-driving phase (like `UPGRADE_PICK`): each player secretly picks exactly one option. Shared **~6s** timer (TBD); timeout auto-commits **Abstain** (never a random card — an unchosen pact must not bind anyone). The commit input is a **positional single-press with last-press-wins**, the same on every surface: the four options (2 dealt cards + Abstain + Defiance) render face-up with a per-player key glyph each; one press commits, any later press during the window **replaces** the commit, and the timer locks it — so a mispress is always correctable and no cursor state ever exists. What the press *echoes* depends on client seating (rendering only — input semantics never fork):
   - **Single-human client** (solo local, and every online peer): your own committed card renders **face-up and highlighted** — there is nobody on this screen to hide it from, and you should see exactly what you committed. Since online is always one human per client, this is the **default experience**.
   - **Multi-human client** (shared keyboard): the committed card flips to a face-down "LOCKED IN" state — no identity shown. No cursor, no focus highlight, no cycling; nothing on screen reveals the pick. This blind rendering is the degraded mode for the rare shared-keyboard case, not the baseline.
   (Touch clients on their own screen tap the card directly — private because the screen is.) Other players' commits are hidden from every client and from AI view slices until reveal.
...
- **No-repeat (v1 rotation rule)** — the pair that formed a pact in round N cannot match in round N+1: the alliance geometry must rotate every round (new pair, or free-for-all), mirroring the modifier no-consecutive-repeat. Enforced at the **dealer**: neither player of `lastPactPair` is dealt a card that could match with the other (their reciprocal/mirror space excludes each other), so no hand is wasted on an impossible match (a reveal-time fizzle check backstops the dealer). State: `lastPactPair`, same shape as `lastModifierId`. The Sworn Alliances layer (future directions) **replaces** this rule — renewal escalation and forced rotation are mutually exclusive designs; v1 ships rotation.
...
first-2-grunts auto-kill in both zones) · Materiel Convoy (reciprocal build
card: during the next Wall Build each partner may send their *current piece*
to the partner's bag front — a "gift" input, capped at 2 sends per player;
pure bag operation, so zones stay sealed — the only tactile cooperative verb
in the catalog, everything else is a buff or a re-price) · Land Grant (+25% territory if both
...
- **Sworn Alliances (the ratchet)** — the same pair matching in consecutive rounds escalates the pact: the bonus grows per renewal *and so does the betrayal payout* (betraying a 3-round alliance keeps part of the accumulated pot instead of forfeiting). Long alliances become mutual dares — inherently unstable by design, which enforces the governing principle structurally. **Adopting this REPLACES the v1 no-repeat rotation rule** (they are mutually exclusive: forced rotation forbids the consecutive matches the ratchet is built on) — shipping it is an explicit decision to trade "alliances change every round" for "alliances age and explode."
...
- **Cycling-cursor commit UI** (the upgrade-pick dialog, reused): on a shared keyboard a rendered focus leaks the pick outright, and even unrendered *cycling* leaks through press-counting. Superseded by the blind positional single-press (one key, one press, no cursor — nothing to count), which is now the universal commit input and what removes the seating gate entirely. Public commits stay rejected on any surface: they turn simultaneous choice into a last-second hover-chicken game — a different (worse) mechanic. What secrecy cannot be on a shared screen is *persistent* — pacts are secret for the ~6s commit window and public forever after, and no design that needs longer-lived hidden state fits local play.
...
- **State** (`ModernState`): `pactHands` (dealt, public), `pactProposals` (public), `pactCommits` (hidden-until-reveal), `activePacts` (post-reveal, incl. violation flags), `lastPactPair` (feeds the dealer's no-repeat exclusion; survives into the next round like `lastModifierId`), payout accumulators. All but `lastPactPair` reset in `prepareNextRound` with the other one-round effects.
- **Hidden info**: commits are display-gated only (clients are trusted; server is a relay — precedent: `SupplyShip.bonus` is hidden until sunk). Hard rule: unrevealed commits must be excluded from AI view slices (`BattleViewState` etc.) — "AI uses human-visible info only". Commit-reveal hashing is a known upgrade if the trust model ever changes; explicitly out of scope. Display-gating hides a commit per **client**, not per player — hidden-until-reveal always applies to *other* players' commits, while a client's **own** human's commit is face-up whenever that client seats exactly one human (nothing to hide from; keyed off a fact the client already knows, zero sim state). On a shared keyboard the blind positional press keeps the pick out of the display entirely, so multi-human clients are safe (multi-human play is keyboard-driven; a shared-screen tap would be watchable, but that configuration is dev-only — accepted, not gated).
- **Wire**: two input messages (`PACT_PROPOSE`, `PACT_COMMIT`) — uncomputable human input, everything else mirror-simulated. `PACT_COMMIT` is **latest-wins** per player within the window (re-commits simply overwrite; nothing binds until reveal, so the relay needs no ordering guarantees beyond per-sender order). Deal uses synced RNG at cannon entry (no wire traffic). Resolved pacts ride the BATTLE_START checkpoint; mid-window FULL_STATE snapshots must carry hands/proposals/commits (same class as `FullStateMessage.roundEnd` routing — the `checkpoint-fields` lint will demand this).
...
- **Input**: the commit is a positional single-press — each of the 4 options binds to one key from the player's existing `PLAYER_KEY_BINDINGS` set (e.g. Left / Right / Down / Confirm), rendered as a glyph on the card; no cursor state exists. **Last-press-wins**: every press during the window overwrites the pending commit and the timer expiry locks it (uniform across seatings — only the *echo* varies, per the Commit step). Same input path online and local; touch clients (own screen) tap the card. The **only genuinely new input surface** is the optional step-1 proposal during Cannon Place (cycle-stance key + a tap chip on touch) — the known-expensive part; keep it one control.
