...
- **Phase entry is owned by `game/`.** All `state.phase` mutation flows through `setPhase` in `phase-setup.ts`, wrapped by an `enter*Phase` helper in `phase-entry.ts` (one per phase: `enterCannonPhase`, `enterModifierRevealPhase`, `enterBattlePhase`, `enterUpgradePickPhase`, `enterWallBuildPhase`, `enterSelectionPhase`, `enterReselectPhase`). Each helper sets the phase + primes any entry-time `state.timer` value. `enterSelectionPhase` and `enterReselectPhase` both flip to `CASTLE_SELECT` — they differ only in scope (all active players vs the queued reselectors); cycle type is derived from `state.round`. Runtime transitions (in `runtime-phase-machine.ts`) call these helpers from their `mutate` — they do NOT call `setPhase` directly or write `state.phase` / entry-time `state.timer` inline. The deep-import allowlist in `scripts/lint-restricted-imports.ts` permits `setPhase` for two `online/` checkpoint-replay sites only; everything else goes through `game/index.ts`.
