...
- MCP play server (dev/research): `deno run -A scripts/mcp-play/server.ts` — an MCP stdio server that lets an external LLM agent play a classic match by driving ONE slot through the real controller/intent path (an `McpBrain` is brain-swapped onto the agent slot via `RuntimeConfig.controllerFactory`; the other slots stay default AI). Turn-based `observe`/`act`/`pass` over the headless mock clock, which is frozen while the agent "thinks" and burns a fixed per-action time budget when it acts. Impl: `scripts/mcp-play/harness.ts` (`createMcpGame`). Tools: observe, check_placement, select_castle, place_cannon/end_cannon, place_piece, fire, `build_toward`/`bombard` (intent actions = one call per build/battle), pass, enclose_plan, save/load. NOT in determinism/parity suites (the agent slot is non-deterministic by design). **Fairness invariant:** any agent action whose COUNT is bounded by game-time — not by slots or reload — must be charged the *real* per-action cost, or the frozen-clock model becomes an exploit. Two such levers exist and must stay honest: build pieces cost `BUILD_PIECE_TICKS` (the measured AI per-piece cadence, ~78t), and firing is gated to the live battle window (`battleCountdown <= 0 && timer > 0`, matching `ai-phase-battle.ts`) so the agent can't out-build or out-fire opponents on clock speed. Cannon placement (slot-capped) and fire rate (reload-capped) are self-limiting and need no extra charge.
